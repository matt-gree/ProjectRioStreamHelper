...
| `settings._deep_merge` | `test_settings_logic.py` | loaded overrides defaults, missing default keys preserved, nested dict merge, non-dict-over-dict replacement; `_USER_OWNED_MAPS` paths replaced wholesale (seed only when absent, keyed on dotted path, siblings still merge). |
