...
def test_stars_won_out_goes_to_fielding_team():
    ...
    assert PostGame._stars_won([ev(0, "Out")]) == (0, 1)
    # Bottom (half 1): home bats, out goes to away.
    assert PostGame._stars_won([ev(1, "Strikeout")]) == (1, 0)


def test_stars_won_hit_goes_to_batting_team():
    assert PostGame._stars_won([ev(0, "Single")]) == (1, 0)
    assert PostGame._stars_won([ev(1, "HR")]) == (0, 1)


def test_stars_won_walks_go_to_batting_team():
    # A walk puts the batter on — the batting team keeps the star.
    assert PostGame._stars_won([ev(0, "Walk (BB)"), ev(0, "Walk (HBP)")]) == (2, 0)
    assert PostGame._stars_won([ev(1, "Walk (BB)")]) == (0, 1)
...
def test_linescore_attributes_runs_to_innings():
    ...
    away, home = PostGame._linescore(events, _FinalScore(3, 2))
    assert away == [1, 0, 2]
    assert home == [0, 2, 0]
...
def test_linescore_marks_unplayed_bottom_half_as_x():
    ...
    away, home = PostGame._linescore(events, _FinalScore(1, 3))
    assert away == [0, 1]
    assert home == [3, None]
...
def test_linescore_reconciles_walkoff_runs_after_last_event():
    ...
    events = [lev(0, 1, 0, 2, 0), lev(1, 1, 1, 2, 3), lev(2, 2, 0, 2, 3),
              lev(3, 2, 1, 2, 3)]
    away, home = PostGame._linescore(events, _FinalScore(2, 4))
    assert away == [2, 0]
    assert home == [3, 1]


def test_linescore_tolerates_junk():
    assert PostGame._linescore([], _FinalScore(0, 0)) == ([], [])
    assert PostGame._linescore(None, _FinalScore(0, 0)) == ([], [])
    events = ["bogus", {"Inning": "x"}, lev(0, 1, 0, None, "n/a"), lev(1, 1, 1, 1, 0)]
    away, home = PostGame._linescore(events, _FinalScore(1, 0))
    assert away == [1]
    assert home == [0]
...
def test_side_totals_shape():
    totals = PostGame._side_totals(_StubStat(), 0, stars_won=4)
    assert totals == {
        "runs": 7,
        "hits": 11,
        "homeruns": 2,
    ...
    totals = PostGame._side_totals(_StubStat(), 1, stars_won=0)
