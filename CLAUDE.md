...
| One board's console panel (transport readout, `side_reason`, corrections, rename/remove) | `src/routes/production/desks/board.jsx` + `boards.js` (`boardDeskId`, `useActiveBoards`), rows in `rack.jsx` (`useDeskRows`), bodies in `production.jsx` (`deskBodiesFor`), face in `quickface.jsx` (`deskQuickFace`) |
