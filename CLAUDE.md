...
| Match queue (membership, order, "up next" per board) | `server/schedule.py` (`append`, `move`, `next_up`, `is_up_next_eligible`), `server/api/v1/schedule.py` (per-id queue verbs), `take_next_match` in `server/api/v1/match.py`; client `src/context/schedule.js` + `src/routes/production/queue.js` (`useQueueOrder`), authored on `desks/match.jsx`, taken on `desks/board.jsx` + `quickface.jsx`. The Upcoming Schedule stage panel (`stage/schedule.jsx`) is display-only — heading + per-match time |
