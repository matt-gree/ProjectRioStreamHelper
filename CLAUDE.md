...
| What the console calls side 1 and side 2 | `src/routes/production/sides.js` — `sideLabel` (standalone: an eyebrow, a control) and `sidePhrase` (mid-sentence, carries its article), both off `production.side_labels` (`numeric` \| `lr` \| `tb`, default numeric), authored in Settings → Production. **Every surface that names a side goes through it**: the board desk's columns, score boxes and `sideReasonLine`, the subject rows, the container scope control, the controller URLs, the match draft cards, the Player Lock captions, and the `?team=` variant label in `instances.js` / `addsource.js` — so a rack row and the panel it opens cannot disagree. **Geometry keeps its literal words**: the container anchor menu, `PP_LOCATION_OPTIONS`, `?dir=left\|right` and text alignment describe a position on a canvas, not a side of a game. `project_rio.pinned_side` stores the string `"Team 1"`/`"Team 2"` and two server readers compare against it — only its caption moves |
...
| Per-side stat card (the Stats element) | `public/layout/scoreboard1/stats.html` → `lib/stats-card-mount.js`, registered `direct` + `perSide` in `src/routes/production/elements.js`, settings `overlays.stats.*` (`STAT_CARD_SETTINGS`). **`stats` used to name a different overlay**: a fed 325×120 DOM bar (`lib/stats-mount.js`) in a seeded "Stats Bar" container, with a picker for choosing a roster character by hand — while the overlay a broadcast actually uses matched no element and rowed generically. The fed bar is shelved (layout and mount still on disk, nothing offers them); `statscard` is what survives of "a card inside a container" and is the same data in the themed 2×2 art under its own namespace |
