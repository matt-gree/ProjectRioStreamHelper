...
- **A setting whose PART only exists at some SIZES declares `sizes` on its def.** The whitelist is per layout *type* and the scoreboard's three sizes are one HTML file with one `<meta>`, so that gate structurally cannot see the difference — `settingReachesSize` (`designConstants.js`) is where it is drawn, over `sizeCodeFor`, which is the one statement of the mount's "unknown or absent `?size=` resolves to `l`" fallback. The stage passes the source's own size (`stage/index.jsx`), so the row is dropped exactly where nothing would move: ELO on the Small board, which has no completed-game cluster and no `elo*` slots in either shipped package. Sibling of `showWhen` and `appPalette` — the other two ways a registered setting is inert on the source in front of you. The value is stored per **board**, so dropping the row never disturbs what that board's other sizes show.
